...
object PostgresEventStoreSuite extends IOSuite:
  ...
  test("append stores event with correct payload, tags, and event type") { case components =>
    val store = components.eventStore
    for
      id     <- freshId("student")
      tag     = Tag("student", id)
  ...
  test("append round-trips non-empty headers through the store") { case components =>
    val store = components.eventStore
  ...
  test("append defaults to empty headers when none are provided") { case components =>
    val store = components.eventStore
    for
      id     <- freshId("student")
      tag     = Tag("student", id)
  ...
  test("a multi-event append keeps each event's own headers, tags and isExternal") { case components =>
    val store = components.eventStore
  ...
  test("a duplicate UUID within a single append is written once and shares the same metadata") { case components =>
    val store = components.eventStore
    for
  ...
  test("readFrom skips events at or before the given position") { case components =>
    val store = components.eventStore
  ...
  test("readFrom with empty event type filter matches all event types") { case components =>
    val store = components.eventStore
    for
      id     <- freshId("student")
      tag     = Tag("student", id)
  ...
  test("concurrent appends with the same tag allow only one success") { case components =>
    val store = components.eventStore
    for
      id             <- freshId("student")
      tag             = Tag("student", id)
  ...
  test("concurrent appends with overlapping tag sets allow only one success") { case components =>
    val store = components.eventStore
    for
  ...
  test("many concurrent appends with distinct tags all succeed") { case components =>
    val store = components.eventStore
  ...
  test("appends with fresh tags can start from expected index zero after prior events") { case components =>
    val store = components.eventStore
    for
  ...
  test("append rejects an expected index ahead of the scoped revision") { case components =>
    val store = components.eventStore
    for
  ...
  test("append with empty tags uses all tags for conflict detection") { case components =>
    val store = components.eventStore
    for
      firstTag  <- freshId("first").map(id => Tag("student", id))
      secondTag <- freshId("second").map(id => Tag("student", id))
  ...
  test("notify completes without error") { case components =>
    components.sendNotification(EventStoreNotification.PauseProjection("test-proj")).as(success)
  }

  test("appending with a provided UUID sets isExternal to true") { case components =>
    val store = components.eventStore
    for
      id  <- freshId("external")
      tag  = Tag("external", id)
  ...
  test("appending without a provided UUID sets isExternal to false") { case components =>
    val store = components.eventStore
    for
      id     <- freshId("internal")
      tag     = Tag("internal", id)
  ...
  test("appending a duplicate UUID is a no-op and returns the original global position") { case components =>
    val store = components.eventStore
    for
      id  <- freshId("dedup")
      tag  = Tag("dedup", id)
  ...
  test("appendUnchecked stores event with correct payload, tags, event type, and isExternal") { case components =>
    val store = components.eventStore
    for
      id     <- freshId("imported")
      tag     = Tag("imported", id)
  ...
  test("concurrent appendUnchecked calls with the same tag all succeed (no OCC)") { case components =>
    val store = components.eventStore
  ...
  test("appendUnchecked with a duplicate UUID is a no-op and returns the original global position") { case components =>
    val store = components.eventStore
    for
  ...
  test("appendUnchecked with no events is a no-op") { case components =>
    val store = components.eventStore
    for
