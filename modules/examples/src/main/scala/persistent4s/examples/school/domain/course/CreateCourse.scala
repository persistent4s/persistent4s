...
object CreateCourseHandler extends CommandHandler[CreateCourse, CreateCourseState, SchoolEvent]:
  ...
  def evolve(command: CreateCourse, state: CreateCourseState, event: SchoolEvent): CreateCourseState =
    event match
      case _: CourseCreated => state.copy(exists = true)
      case _                => state

  def validate(state: CreateCourseState, command: CreateCourse): Either[Throwable, Unit] =
    if (state.exists) Left(new Exception("Course already exists")) else Right(())
