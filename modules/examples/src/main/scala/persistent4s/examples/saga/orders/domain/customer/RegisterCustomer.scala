...
import persistent4s.examples.saga.orders.domain.{CustomerRegistered, OrdersEvent, OrdersTags}

final case class RegisterCustomer(customerId: UUID, name: String)

final case class RegisterCustomerState(exists: Boolean)

object RegisterCustomerHandler extends CommandHandler[RegisterCustomer, RegisterCustomerState, OrdersEvent]:
  ...
  def evolve(command: RegisterCustomer, state: RegisterCustomerState, event: OrdersEvent): RegisterCustomerState =
    event match
  ...
  def decide(state: RegisterCustomerState, command: RegisterCustomer): List[(Set[Tag], OrdersEvent)] =
    List(
